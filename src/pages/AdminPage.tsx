...
import { ZusatzleistungenViewDialog } from '@/components/dialogs/ZusatzleistungenViewDialog';
...
import { KatzenViewDialog } from '@/components/dialogs/KatzenViewDialog';
...
const ZUSATZLEISTUNGEN_FIELDS = [
  { key: 'leistungsname', label: 'Name der Leistung', type: 'string/text' },
...
const KATZEN_FIELDS = [
  { key: 'katzenname', label: 'Name der Katze', type: 'string/text' },
  ...
  { key: 'impfstatus', label: 'Impfstatus', type: 'lookup/select', options: [{ key: 'vollstaendig_geimpft', label: 'Vollständig geimpft' }, { key: 'teilweise_geimpft', label: 'Teilweise geimpft' }, { key: 'nicht_geimpft', label: 'Nicht geimpft' }, { key: 'unbekannt_impf', label: 'Unbekannt' }] },
  { key: 'besonderheiten', label: 'Besonderheiten / Gesundheitshinweise', type: 'string/textarea' },
  { key: 'besitzer', label: 'Besitzer', type: 'applookup/select', targetEntity: 'kunden', targetAppId: 'KUNDEN', displayField: 'nachname' },
];
const BUCHUNGEN_FIELDS = [
  { key: 'gesamtpreis', label: 'Gesamtpreis (€)', type: 'number' },
  { key: 'katze', label: 'Katze', type: 'applookup/select', targetEntity: 'katzen', targetAppId: 'KATZEN', displayField: 'katzenname' },
  { key: 'anreise', label: 'Anreisedatum', type: 'date/date' },
  { key: 'abreise', label: 'Abreisedatum', type: 'date/date' },
  { key: 'unterkunftstyp', label: 'Unterkunftstyp', type: 'lookup/select', options: [{ key: 'standard', label: 'Standardzimmer' }, { key: 'komfort', label: 'Komfortzimmer' }, { key: 'suite', label: 'Suite' }] },
  { key: 'kunde', label: 'Kunde', type: 'applookup/select', targetEntity: 'kunden', targetAppId: 'KUNDEN', displayField: 'nachname' },
  { key: 'zusatzleistungen', label: 'Zusatzleistungen', type: 'multipleapplookup/select', targetEntity: 'zusatzleistungen', targetAppId: 'ZUSATZLEISTUNGEN', displayField: 'leistungsname' },
  { key: 'status', label: 'Buchungsstatus', type: 'lookup/select', options: [{ key: 'angefragt', label: 'Angefragt' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'eingecheckt', label: 'Eingecheckt' }, { key: 'ausgecheckt', label: 'Ausgecheckt' }, { key: 'storniert', label: 'Storniert' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
  { key: 'preis_pro_nacht', label: 'Preis pro Nacht (€)', type: 'number' },
];

const ENTITY_TABS = [
  { key: 'zusatzleistungen', label: 'Zusatzleistungen', pascal: 'Zusatzleistungen' },
  { key: 'kunden', label: 'Kunden', pascal: 'Kunden' },
  { key: 'katzen', label: 'Katzen', pascal: 'Katzen' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('zusatzleistungen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'zusatzleistungen': new Set(),
    'kunden': new Set(),
    'katzen': new Set(),
    'buchungen': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'zusatzleistungen': return (data as any).zusatzleistungen as Zusatzleistungen[] ?? [];
      case 'kunden': return (data as any).kunden as Kunden[] ?? [];
      case 'katzen': return (data as any).katzen as Katzen[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'katzen':
        ...
        break;
      case 'buchungen':
        lists.katzenList = (data as any).katzen ?? [];
        lists.kundenList = (data as any).kunden ?? [];
        lists.zusatzleistungenList = (data as any).zusatzleistungen ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'katzen' && fieldKey === 'besitzer') {
      const match = (lists.kundenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.nachname ?? '—';
    }
    if (entity === 'buchungen' && fieldKey === 'katze') {
      const match = (lists.katzenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.katzenname ?? '—';
    }
    if (entity === 'buchungen' && fieldKey === 'kunde') {
      const match = (lists.kundenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.nachname ?? '—';
    }
    if (entity === 'buchungen' && fieldKey === 'zusatzleistungen') {
      const match = (lists.zusatzleistungenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'zusatzleistungen': return ZUSATZLEISTUNGEN_FIELDS;
      case 'kunden': return KUNDEN_FIELDS;
      case 'katzen': return KATZEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'zusatzleistungen': return {
        create: (fields: any) => LivingAppsService.createZusatzleistungenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateZusatzleistungenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteZusatzleistungenEntry(id),
      };
      case 'kunden': return {
      ...
      case 'katzen': return {
        create: (fields: any) => LivingAppsService.createKatzenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKatzenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKatzenEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'zusatzleistungen' || dialogState?.entity === 'zusatzleistungen') && (
        <ZusatzleistungenDialog
          open={createEntity === 'zusatzleistungen' || dialogState?.entity === 'zusatzleistungen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'zusatzleistungen' ? handleUpdate : (fields: any) => handleCreate('zusatzleistungen', fields)}
          defaultValues={dialogState?.entity === 'zusatzleistungen' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'katzen' || dialogState?.entity === 'katzen') && (
        <KatzenDialog
          open={createEntity === 'katzen' || dialogState?.entity === 'katzen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'katzen' ? handleUpdate : (fields: any) => handleCreate('katzen', fields)}
          defaultValues={dialogState?.entity === 'katzen' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'buchungen' || dialogState?.entity === 'buchungen') && (
        <BuchungenDialog
          ...
          katzenList={(data as any).katzen ?? []}
          kundenList={(data as any).kunden ?? []}
          zusatzleistungenList={(data as any).zusatzleistungen ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Buchungen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchungen']}
        />
      )}
      {viewState?.entity === 'zusatzleistungen' && (
        <ZusatzleistungenViewDialog
          open={viewState?.entity === 'zusatzleistungen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'zusatzleistungen', record: r }); }}
        />
      )}
      ...
      {viewState?.entity === 'katzen' && (
        <KatzenViewDialog
          open={viewState?.entity === 'katzen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'katzen', record: r }); }}
          kundenList={(data as any).kunden ?? []}
        />
      )}
