...
import { formEnhancements } from '@/config/form-enhancements/Katzen';
import { evalComputed } from '@/config/form-enhancements/types';

export default function KatzenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Katzen | null>(null);
  ...
  async function handleUpdate(fields: Katzen['fields']) {
    if (!record) return;
    await LivingAppsService.updateKatzenEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteKatzenEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/katzen');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/katzen')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/katzen')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.katzenname ?? 'Katzen'} />
      ...
      <RecordSection title="Details" cols={2}>
        ...
        <RecordField label="Besitzer" value={getKundenDisplayName(record.fields.besitzer)} format="text" />
      </RecordSection>
