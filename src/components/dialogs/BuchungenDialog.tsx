...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  unterkunftstyp: LOOKUP_OPTIONS['buchungen']?.['unterkunftstyp'] ?? [],
  status: LOOKUP_OPTIONS['buchungen']?.['status'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  katze: APP_IDS.KATZEN,
  kunde: APP_IDS.KUNDEN,
  zusatzleistungen: APP_IDS.ZUSATZLEISTUNGEN,
};
...
export function BuchungenDialog({ open, onClose, onSubmit, defaultValues, recordId, katzenList, kundenList, zusatzleistungenList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['katze', 'anreise', 'abreise', 'kunde', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">Buchungsstatus <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
