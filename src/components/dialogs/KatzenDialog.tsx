...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  geschlecht: LOOKUP_OPTIONS['katzen']?.['geschlecht'] ?? [],
  impfstatus: LOOKUP_OPTIONS['katzen']?.['impfstatus'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  besitzer: APP_IDS.KUNDEN,
};
...
export function KatzenDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: KatzenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['katzenname', 'besitzer'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Katzen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'katzen');
      await onSubmit(clean as Katzen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "katzenname": string | null, // Name der Katze\n  "rasse": string | null, // Rasse\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "weiblich" | "maennlich" | "unbekannt") mapping: weiblich=Weiblich, maennlich=Männlich, unbekannt=Unbekannt\n  "farbe": string | null, // Fellfarbe\n  "impfstatus": LookupValue | null, // Impfstatus (select one key: "vollstaendig_geimpft" | "teilweise_geimpft" | "nicht_geimpft" | "unbekannt_impf") mapping: vollstaendig_geimpft=Vollständig geimpft, teilweise_geimpft=Teilweise geimpft, nicht_geimpft=Nicht geimpft, unbekannt_impf=Unbekannt\n  "besonderheiten": string | null, // Besonderheiten / Gesundheitshinweise\n  "besitzer": string | null, // Display name from Kunden (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        if (besitzerName) {
          const besitzerMatch = kundenList.find(r => matchName(besitzerName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (besitzerMatch) merged['besitzer'] = createRecordUrl(APP_IDS.KUNDEN, besitzerMatch.record_id);
        }
        return merged as Partial<Katzen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Katzen bearbeiten' : 'Katzen hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'katzenname': (
      <div key="katzenname" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.katzenname && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'rasse': (
      <div key="rasse" className="space-y-1.5">
    ...
    'geschlecht': (
      <div key="geschlecht" className="space-y-1.5">
        <Label htmlFor="geschlecht">Geschlecht</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.geschlecht) === 'weiblich'}
            onClick={() => setFields(f => ({ ...f, geschlecht: (lookupKey(f.geschlecht) === 'weiblich' ? undefined : 'weiblich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.geschlecht) === 'weiblich'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.geschlecht) === 'maennlich'}
            onClick={() => setFields(f => ({ ...f, geschlecht: (lookupKey(f.geschlecht) === 'maennlich' ? undefined : 'maennlich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.geschlecht) === 'maennlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.geschlecht) === 'unbekannt'}
            onClick={() => setFields(f => ({ ...f, geschlecht: (lookupKey(f.geschlecht) === 'unbekannt' ? undefined : 'unbekannt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.geschlecht) === 'unbekannt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'farbe': (
      <div key="farbe" className="space-y-1.5">
    ...
    'impfstatus': (
      <div key="impfstatus" className="space-y-1.5">
        <Label htmlFor="impfstatus">Impfstatus</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'vollstaendig_geimpft'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'vollstaendig_geimpft' ? undefined : 'vollstaendig_geimpft') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'vollstaendig_geimpft'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'teilweise_geimpft'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'teilweise_geimpft' ? undefined : 'teilweise_geimpft') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'teilweise_geimpft'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'nicht_geimpft'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'nicht_geimpft' ? undefined : 'nicht_geimpft') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'nicht_geimpft'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.impfstatus) === 'unbekannt_impf'}
            onClick={() => setFields(f => ({ ...f, impfstatus: (lookupKey(f.impfstatus) === 'unbekannt_impf' ? undefined : 'unbekannt_impf') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.impfstatus) === 'unbekannt_impf'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'besonderheiten': (
      <div key="besonderheiten" className="space-y-1.5">
    ...
    'besitzer': (
      <div key="besitzer" className="space-y-1.5">
        ...
        <Combobox
          ...
          onChange={id => setFields(f => ({ ...f, besitzer: id ? createRecordUrl(APP_IDS.KUNDEN, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateKunden("besitzer", q)}
  ...
  const FIELD_LABELS: Record<string, string> = {"katzenname": "Name der Katze", "rasse": "Rasse", "geburtsdatum": "Geburtsdatum", "geschlecht": "Geschlecht", "farbe": "Fellfarbe", "impfstatus": "Impfstatus", "besonderheiten": "Besonderheiten / Gesundheitshinweise", "besitzer": "Besitzer"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"besitzer": {"nachname": "Nachname", "telefon": "Telefon", "email": "E-Mail-Adresse", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "vorname": "Vorname"}};
