...
export function ZusatzleistungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ZusatzleistungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['leistungsname'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Zusatzleistungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'zusatzleistungen');
      await onSubmit(clean as Zusatzleistungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Zusatzleistungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Zusatzleistungen bearbeiten' : 'Zusatzleistungen hinzufügen';
  ...
  const FIELD_LABELS: Record<string, string> = {"leistungsname": "Name der Leistung", "beschreibung": "Beschreibung", "preis": "Preis (€)"};
